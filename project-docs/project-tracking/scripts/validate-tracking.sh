...
if [[ ! -f "project.yaml" ]]; then
    print_status $RED "❌ project.yaml not found in current directory"
    exit 1
fi
...
print_status $BLUE "Validating project.yaml..."
python3 - <<'PY'
import json
import sys
...
try:
    import yaml
except ImportError:  # pragma: no cover - dependency notice handled at runtime
...
try:
    ...
    print(f"❌ project.yaml has invalid YAML: {err}")
    sys.exit(1)

if not isinstance(data, dict):
    print('❌ project.yaml must contain a mapping at the root')
    sys.exit(1)

schema_path = Path('schemas/project-schema.json')
if not schema_path.exists():
    print('❌ Schema file schemas/project-schema.json not found')
    sys.exit(1)
...
try:
    ...
    print("⚠️ Missing dependency 'jsonschema'. Install with: pip install jsonschema. Skipping schema validation.")
    sys.exit(0)

validator = jsonschema.Draft7Validator(schema)
errors = sorted(validator.iter_errors(data), key=lambda e: e.path)
if errors:
    print('❌ project.yaml failed schema validation:')
...
print('✅ project.yaml passes schema validation')
PY
