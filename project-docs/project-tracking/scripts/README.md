...
./project-tracker dashboard                # Visual dashboard (HTML)
```
...
### ⚙️ apply-config.py
**Purpose**: Populate tracker files (TODO.json, USER_STORIES.json, HANDOFF.md)
with values from `project.yaml` so onboarding is as simple as editing one config
file. Ideal for single-user + AI CLI workflows.

`project.yaml` carries a `$schema` pointer to `schemas/project-schema.json`. Keep
those fields valid and `./project-tracker validate` will guarantee the config is
sound before any work starts.
...
# Direct usage
python3 scripts/apply-config.py --config path/to/project.yaml
```

**Highlights**:
- Updates core metadata (project name, version, phase) across files
- Inserts project goal, stack, and command references into kickoff tasks
- Refreshes handoff snapshot and command block for AI agents
- Leaves existing phase/task structure intact for further customization
- Safe to rerun whenever `project.yaml` changes
...
pip install pyyaml               # Required for init-config workflow
