...
{:else if timer}
  <section
    class="relative flex min-h-screen flex-col items-center justify-center overflow-hidden text-center bg-base-300"
  >
    ...
    <div class="absolute bottom-6 right-6 z-10 text-right">
      <p class="text-sm font-semibold text-white/90">{timer.gameName}</p>
      <p class="text-xs text-white/50">{timer.sessionId}</p>
    </div>
  </section>
{/if}
