...
--> statement-breakpoint
CREATE INDEX `idx_backup_history_trigger` ON `backup_history` (`trigger`);--> statement-breakpoint
CREATE INDEX `idx_backup_history_timestamp` ON `backup_history` (`timestamp`);--> statement-breakpoint
CREATE INDEX `idx_backup_history_deleted_at` ON `backup_history` (`deleted_at`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `idx_backup_metrics_trigger` ON `backup_metrics` (`trigger`);--> statement-breakpoint
CREATE INDEX `idx_backup_metrics_created_at` ON `backup_metrics` (`created_at`);--> statement-breakpoint
...
CREATE TABLE `cameras` (
	...
	`has_ptz` integer DEFAULT false NOT NULL,
	`has_audio` integer DEFAULT false NOT NULL,
	`has_ir_control` integer DEFAULT false NOT NULL,
	`ir_mode` text DEFAULT 'auto',
	`audio_volume` integer DEFAULT 80,
	`ptz_pan` integer DEFAULT 0,
	`ptz_tilt` integer DEFAULT 0,
	`ptz_zoom` integer DEFAULT 0,
...
CREATE INDEX `idx_cameras_brand` ON `cameras` (`brand`);--> statement-breakpoint
...
CREATE TABLE `game_milestones` (
	...
	`loop_count` integer,
	`auto_dismiss` integer DEFAULT true NOT NULL,
	`created_at` text DEFAULT CURRENT_TIMESTAMP NOT NULL,
	`updated_at` text DEFAULT CURRENT_TIMESTAMP NOT NULL,
	FOREIGN KEY (`game_id`) REFERENCES `games`(`id`) ON UPDATE no action ON DELETE cascade,
...
CREATE TABLE `session` (
	...
	`impersonatedBy` text,
	FOREIGN KEY (`userId`) REFERENCES `user`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`impersonatedBy`) REFERENCES `user`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE TABLE `user` (
	...
	`marketing_opted_in` integer DEFAULT false,
	`password_hash` text,
	`last_login_at` text,
...
CREATE TABLE `permissions` (
	...
	`label` text NOT NULL,
	`category` text NOT NULL,
	`user_type_scope` text DEFAULT 'operator' NOT NULL,
	`description` text,
	`created_at` text DEFAULT CURRENT_TIMESTAMP NOT NULL
);
--> statement-breakpoint
...
CREATE TABLE `session_hints` (
	...
	`count_as_hint` integer DEFAULT true NOT NULL,
